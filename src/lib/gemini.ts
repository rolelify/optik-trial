...
export async function askGemini(data: ViewportData, diffText?: string): Promise<GeminiVerdict> {
  ...
  let prompt = `You are OptikOps, a Visual CI reasoning engine.
...
Primary BBox is: ${data.primaryBBox ? JSON.stringify(data.primaryBBox) : 'null'}

All detected intent elements:
${elementsMarkdown}

Your job is to determine if the primary CTA is occluded, cut off, offscreen, or unclickable.
Look closely at the screenshot. Does a sticky header, modal, or footer overlap the button? Is it clipped by the viewport?
...
Guardrails: Do not hallucinate. If unsure, set failure_type="unknown" and pass=true with low confidence.
